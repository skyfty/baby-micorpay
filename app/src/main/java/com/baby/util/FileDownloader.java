...
public class FileDownloader {
		...
		downloadExecutor.execute(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					STATUS_MAP.put(threadKey, STATUS_FAILED);
					msg = new Message();
					msg.what = STATUS_FAILED;
					handler.sendMessage(msg);
					e.printStackTrace();
	...
	static class DownloadHandler extends Handler {
		...
		public void dispatchMessage(Message message) {
			...
			case STATUS_RUNING:
				...
				if((STATUS_MAP.get(threadKey)!=STATUS_RUNING &&STATUS_MAP.get(threadKey)!=STATUS_DONE) || STATUS_MAP.get(threadKey)==STATUS_FAILED)
				{
					break;
				}
	...
	public interface FileDownloadCallBack {
		void progress(String threadKey, long fileSize, long downloadSize);

		void statusChange(String threadKey, int status);
	}
}
